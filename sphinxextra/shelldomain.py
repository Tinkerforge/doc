...
# REs for Shell signatures
sh_sig_re = re.compile(
    r'''^ (tinkerforge \s+ (?:call|dispatch) \s+)  # prefix
          ([\w-]+) \s+                             # device name
          <uid> \s+
          ([\w-]+) \s*                             # function/callback name
...
class ShellObject(ObjectDescription):
    """
    Description of a general Shell object.
    """
    option_spec = {
    ...
    def handle_signature(self, sig, signode):
        ...
        if s[0] == 'X':
            ...
            modname = self.options.get('module', self.env.temp_data.get('sh:module'))
        ...
        modname = self.options.get('module', self.env.temp_data.get('sh:module'))
        classname = self.env.temp_data.get('sh:class')
        if classname:
            add_module = False
        ...
        # 'exceptions' module.
            ...
            modname = self.options.get(
                'module', self.env.temp_data.get('sh:module'))
    ...
    def add_target_and_index(self, name_cls, sig, signode):
        ...
        if fullname not in self.state.document.ids:
            ...
            objects = self.env.domaindata['sh']['objects']
    ...
    def after_content(self):
        if self.clsname_set:
            self.env.temp_data['sh:class'] = None


class ShellModulelevel(ShellObject):
    """
...
class ShellClasslike(ShellObject):
    """
    ...
    def before_content(self):
        ShellObject.before_content(self)
        if self.names:
            self.env.temp_data['sh:class'] = self.names[0][0]
            self.clsname_set = True


class ShellClassmember(ShellObject):
    """
    ...
    def before_content(self):
        ...
        if lastname and not self.env.temp_data.get('sh:class'):
            self.env.temp_data['sh:class'] = lastname.strip('.')
            self.clsname_set = True


class ShellModule(Directive):
    """
    ...
    def run(self):
        ...
        env.temp_data['sh:module'] = modname
        env.domaindata['sh']['modules'][modname] = \
            (env.docname, self.options.get('synopsis', ''),
             self.options.get('platform', ''), 'deprecated' in self.options)
        # make a duplicate entry in 'objects' to facilitate searching for the
        # module in ShellDomain.find_obj()
        env.domaindata['sh']['objects'][modname] = (env.docname, 'module')
...
class ShellCurrentModule(Directive):
    """
    ...
    def run(self):
        ...
        modname = self.arguments[0].strip()
        if modname == 'None':
            env.temp_data['sh:module'] = None
        else:
            env.temp_data['sh:module'] = modname
        return []


class ShellXRefRole(XRefRole):
    ...
    def process_link(self, env, refnode, has_explicit_title, title, target):
        refnode['sh:module'] = env.temp_data.get('sh:module')
        refnode['sh:class'] = env.temp_data.get('sh:class')
        if not has_explicit_title:
...
class ShellModuleIndex(Index):
    """
    Index subclass to provide the Shell module index.
    """

    name = 'modindex'
    localname = _('Shell Module Index')
    shortname = _('modules')
...
class ShellDomain(Domain):
    """Shell language domain."""
    name = 'sh'
    label = 'Shell'
    object_types = {
        'function':     ObjType(_('function'),      'func', 'obj'),
        'callback':     ObjType(_('callback'),      'cb', 'obj'),
    ...
    directives = {
        ...
        'method':        ShellClassmember,
        'classmethod':   ShellClassmember,
        'staticmethod':  ShellClassmember,
        'attribute':     ShellClassmember,
        'module':        ShellModule,
        'currentmodule': ShellCurrentModule,
    }
    roles = {
        'data':  ShellXRefRole(),
        'exc':   ShellXRefRole(),
        ...
        'const': ShellXRefRole(),
        'attr':  ShellXRefRole(),
        'meth':  ShellXRefRole(fix_parens=True),
        'mod':   ShellXRefRole(),
        'obj':   ShellXRefRole(),
    }
    initial_data = {
    ...
    def find_obj(self, env, modname, classname, name, type, searchmode=0):
        """
        Find a Shell object for "name", perhaps using the given module and/or
    ...
    def resolve_xref(self, env, fromdocname, builder,
        ...
        modname = node.get('sh:module')
        clsname = node.get('sh:class')
