...
# REs for Ruby signatures # Matthias: Add block support
    ...
    def handle_signature(self, sig, signode):
        ...
        * it is added to the full name (return value) if not present
        ...
        if m is None:
            print 'FAILED', sig
            raise ValueError
        name_prefix, name_prefix2, name, arglist, block, retann = m.groups()
        ...
        if not arglist:
            if self.needs_arglist():
                ...
                signode += addnodes.desc_parameterlist()
            if block: # Matthias: Add block support
...
class RubyModule(Directive):
    ...
    def run(self):
        ...
        # module in RubyDomain.find_obj()
        env.domaindata['rb']['objects'][modname] = (env.docname, 'module')
...
class RubyDomain(Domain):
    ...
    directives = {
        ...
        'data':          RubyModulelevel,
        'class':         RubyClasslike,
        'exception':     RubyClasslike,
        'method':        RubyClassmember,
        'classmethod':   RubyClassmember,
        'staticmethod':  RubyClassmember,
        'attribute':     RubyClassmember,
        'module':        RubyModule,
    ...
    roles = {
        'data':  RubyXRefRole(),
        'exc':   RubyXRefRole(),
        'func':  RubyXRefRole(fix_parens=True),
        'class': RubyXRefRole(),
        'const': RubyXRefRole(),
        'attr':  RubyXRefRole(),
        'meth':  RubyXRefRole(fix_parens=True),
        'mod':   RubyXRefRole(),
        'obj':   RubyXRefRole(),
    }
    initial_data = {
