...
class JavaObject(ObjectDescription):
    """Description of a Java language object."""
    ...
    def attach_type(self, node, type):
        ...
        pnode = addnodes.pending_xref(
            '', refdomain='java', reftype='type',
            reftarget=text, modname=None, classname=None)
        pnode['java:parent'] = self.env.temp_data.get('java:parent')
...
#        if obj.visibility != 'public':
    ...
    def add_target_and_index(self, sigobj, sig, signode):
        ...
        self.env.domaindata['java']['objects'].setdefault(name,
    ...
    def before_content(self):
        ...
        if lastname and not self.env.temp_data.get('java:parent'):
            assert isinstance(lastname, NamedDefExpr)
            self.env.temp_data['java:parent'] = lastname.name
    ...
    def after_content(self):
        if self.parentname_set:
            self.env.temp_data['java:parent'] = None
    ...
    def handle_signature(self, sig, signode):
        ...
        parent = self.env.temp_data.get('java:parent')
...
class JavaClassObject(JavaObject):
...
class JavaTypeObject(JavaObject):

    def get_index_text(self, name):
        if self.objtype == 'type':
            return _('%s (Java type)') % name
        return ''
...
class JavaMemberObject(JavaObject):

    def get_index_text(self, name):
        if self.objtype == 'member':
            return _('%s (Java member)') % name
        return ''
...
class JavaFunctionObject(JavaObject):
...
class JavaCurrentNamespace(Directive):
    ...
    def run(self):
        ...
        if self.arguments[0].strip() in ('NULL', '0', 'nullptr'):
            env.temp_data['java:prefix'] = None
        else:
            ...
            try:
                ...
                env.temp_data['java:prefix'] = prefix
        return []
...
class JavaXRefRole(XRefRole):
    ...
    def process_link(self, env, refnode, has_explicit_title, title, target):
        refnode['java:parent'] = env.temp_data.get('java:parent')
...
class JavaDomain(Domain):
    ...
    directives = {
        ...
        'function':     JavaFunctionObject,
        'member':       JavaMemberObject,
        'type':         JavaTypeObject,
        'namespace':    JavaCurrentNamespace
    }
    roles = {
        'class':  JavaXRefRole(),
        'func' :  JavaXRefRole(fix_parens=True),
        'member': JavaXRefRole(),
        'type':   JavaXRefRole()
    }
    initial_data = {
