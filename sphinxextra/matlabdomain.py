...
class NamedDefExpr(DefExpr):
    ...
    def __init__(self, name, visibility, static, final=False):
        self.name = name
...
class MemberObjDefExpr(NamedDefExpr):
    ...
    def __init__(self, name, visibility, static, final, typename, value):
        NamedDefExpr.__init__(self, name, visibility, static, final)
        self.typename = typename
        self.value = value
...
class DefinitionParser(object):
    ...
    def _parse_visibility_static_final(self):
        visibility = 'public'
        if self.match(_visibility_re):
            visibility = self.matched_text
        self.skip_ws()
        static = self.skip_word('static')
        self.skip_ws()
        final = self.skip_word('final')
        return visibility, static, final
    ...
    def parse_member_object(self):
        visibility, static, final = self._parse_visibility_static_final()
        typename = self._parse_type()
        name = self._parse_type()
        self.skip_ws()
        if self.skip_string('='):
            ...
            value = None
        return MemberObjDefExpr(name, visibility, static, final, typename, value)
...
class MATLABObject(ObjectDescription):
    ...
    def attach_modifiers(self, node, obj):
        ...
        if obj.static:
            ...
            node += nodes.Text(' ')
        if obj.final:
            node += addnodes.desc_annotation('final', 'final')
            node += nodes.Text(' ')
    ...
    def add_target_and_index(self, sigobj, sig, signode):
        ...
        self.env.domaindata['matlab']['objects'].setdefault(name,
    ...
    def before_content(self):
        ...
        if lastname and not self.env.temp_data.get('matlab:parent'):
            assert isinstance(lastname, NamedDefExpr)
            self.env.temp_data['matlab:parent'] = lastname.name
    ...
    def after_content(self):
        if self.parentname_set:
            self.env.temp_data['matlab:parent'] = None
    ...
    def handle_signature(self, sig, signode):
        ...
        parent = self.env.temp_data.get('matlab:parent')
...
class MATLABClassObject(MATLABObject):

    def get_index_text(self, name):
        return _('%s (MATLAB class)') % name
...
class MATLABTypeObject(MATLABObject):

    def get_index_text(self, name):
        if self.objtype == 'type':
            return _('%s (MATLAB type)') % name
        return ''
...
class MATLABMemberObject(MATLABObject):

    def get_index_text(self, name):
        if self.objtype == 'member':
            return _('%s (MATLAB member)') % name
        return ''
...
class MATLABFunctionObject(MATLABObject):
...
class MATLABCurrentNamespace(Directive):
    ...
    def run(self):
        ...
        if self.arguments[0].strip() in ('NULL', '0', 'nullptr'):
            env.temp_data['matlab:prefix'] = None
        else:
            ...
            try:
                ...
                env.temp_data['matlab:prefix'] = prefix
        return []
...
class MATLABXRefRole(XRefRole):
    ...
    def process_link(self, env, refnode, has_explicit_title, title, target):
        refnode['matlab:parent'] = env.temp_data.get('matlab:parent')
...
class MATLABDomain(Domain):
    ...
    directives = {
        'class':        MATLABClassObject,
        'function':     MATLABFunctionObject,
        'member':       MATLABMemberObject,
        'type':         MATLABTypeObject,
        'namespace':    MATLABCurrentNamespace
    }
    roles = {
        'class':  MATLABXRefRole(),
        'func' :  MATLABXRefRole(fix_parens=True),
        'member': MATLABXRefRole(),
        'type':   MATLABXRefRole()
    }
    initial_data = {
