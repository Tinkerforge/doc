...
# REs for Modbus signatures
modbus_sig_re = re.compile(
    r'''^ ([\w.]*\.)?            # class name(s)
          (\w+)  \s*             # thing name
          (?: \((.*)\)           # optional: arguments
...
modbus_paramlist_re = re.compile(r'([\[\],])')  # split at '[', ']' and ','
    ...
    def handle_signature(self, sig, signode):
        ...
        * it is added to the full name (return value) if not present
        ...
        modname = self.options.get(
            'module', self.env.temp_data.get('modbus:module'))
...
class ModbusModule(Directive):
    ...
    def run(self):
        ...
        env.domaindata['modbus']['modules'][modname] = \
...
class ModbusModuleIndex(Index):
    ...
    name = 'modindex'
    localname = _('Modbus Module Index')
    shortname = _('modules')
...
class ModbusDomain(Domain):
    ...
    directives = {
        'function':      ModbusModulelevel,
        'data':          ModbusModulelevel,
        'class':         ModbusClasslike,
        'exception':     ModbusClasslike,
        'method':        ModbusClassmember,
        'classmethod':   ModbusClassmember,
        'staticmethod':  ModbusClassmember,
        'attribute':     ModbusClassmember,
        'module':        ModbusModule,
        'currentmodule': ModbusCurrentModule,
    }
    roles = {
        'data':  ModbusXRefRole(),
        'exc':   ModbusXRefRole(),
        'func':  ModbusXRefRole(fix_parens=True),
        'class': ModbusXRefRole(),
        'const': ModbusXRefRole(),
        'attr':  ModbusXRefRole(),
        'meth':  ModbusXRefRole(fix_parens=True),
        'mod':   ModbusXRefRole(),
        'obj':   ModbusXRefRole(),
    }
    initial_data = {
