...
# REs for JavaScript signatures
javascript_sig_re = re.compile(
    r'''^ ((?:new)[ ])?\s*
          ([\w.]*\.)?            # class name(s)
          (\w+)  \s*             # thing name
          (?: \((.*)\)           # optional: arguments
...
class JavaScriptObject(ObjectDescription):
    ...
    doc_field_types = [
        ...
        TypedField('returnvalue', label=l_('Callback'),
                   names=('returns', 'return', 'response'),
                   typerolename='obj', typenames=('paramtype', 'type')),
        Field('noreturnvalue', label=l_('Callback'), has_arg=False,
              names=('noreturn',)),
    ...
    def handle_signature(self, sig, signode):
        ...
        * it is added to the full name (return value) if not present
        ...
        modname = self.options.get(
            'module', self.env.temp_data.get('javascript:module'))
...
class JavaScriptModule(Directive):
    ...
    def run(self):
        ...
        ret = []
        if not noindex:
            env.domaindata['javascript']['modules'][modname] = \
...
class JavaScriptDomain(Domain):
    ...
    directives = {
        'function':        JavaScriptModulelevel,
        'data':            JavaScriptModulelevel,
        'class':           JavaScriptClasslike,
        'exception':       JavaScriptClasslike,
        'method':          JavaScriptClassmember,
        'classmethod':     JavaScriptClassmember,
        'staticmethod':    JavaScriptClassmember,
        'attribute':       JavaScriptClassmember,
        'module':          JavaScriptModule,
        'currentmodule':   JavaScriptCurrentModule,
        'decorator':       JavaScriptDecoratorFunction,
        'decoratormethod': JavaScriptDecoratorMethod,
    }
    roles = {
        'data':  JavaScriptXRefRole(),
        'exc':   JavaScriptXRefRole(),
        'func':  JavaScriptXRefRole(fix_parens=True),
        'class': JavaScriptXRefRole(),
        'const': JavaScriptXRefRole(),
        'attr':  JavaScriptXRefRole(),
        'meth':  JavaScriptXRefRole(fix_parens=True),
        'mod':   JavaScriptXRefRole(),
        'obj':   JavaScriptXRefRole(),
    }
    initial_data = {
